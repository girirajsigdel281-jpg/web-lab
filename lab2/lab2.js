// =============================================
// LAB 2 - INTRODUCTION TO JAVASCRIPT (Improved)
// =============================================

// 1. VARIABLES: let, const
console.log("--- 1. Variables ---");

const name = "Lincoln";      // constant value
let age = 20;              // can be reassigned
const country = "Nepal";   // constant value

console.log(`Name: ${name}`);
console.log(`Age: ${age}`);
console.log(`Country: ${country}`);

// let can be changed:
age = 21;
console.log(`Updated age: ${age}`);


// 2. FUNCTIONS
console.log("\n--- 2. Functions ---");

// Function declaration with default parameter
function greet(personName = "Guest") {
  return `Hello, ${personName}!`;
}

// Function expression
const add = function(a, b) {
  return a + b;
};

console.log(greet("Bob"));
console.log(`2 + 3 = ${add(2, 3)}`);


// 3. ARROW FUNCTIONS
console.log("\n--- 3. Arrow Functions ---");

const multiply = (a, b) => a * b;
const double = x => x * 2;
const greetArrow = personName => {
  const message = `Hi there, ${personName}!`;
  return message;
};

console.log(`3 * 4 = ${multiply(3, 4)}`);
console.log(`double(7) = ${double(7)}`);
console.log(greetArrow("Charlie"));


// 4. OBJECTS
console.log("\n--- 4. Objects ---");

const student = {
  name: "Lumen",
  age: 19,
  college: "TU",
  isActive: true,
  introduce() {
    return `I am ${this.name}, age ${this.age}`;
  }
};

console.log(`Student name: ${student.name}`);
console.log(`Student age: ${student.age}`);
console.log(student.introduce());

student.age = 20; // update property
student.gpa = 3.8; // add new property

console.log(`After update: ${student.age}`);
console.log(`GPA added: ${student.gpa}`);

// Destructuring
const { name: sName, college } = student;
console.log(`Destructured: ${sName} from ${college}`);


// 5. ARRAYS + MAP
console.log("\n--- 5. Arrays + map() ---");

const numbers = [1, 2, 3, 4, 5];
const squared = numbers.map(n => n ** 2);
const doubled = numbers.map(double);

console.log("Original:", numbers);
console.log("Squared:", squared);
console.log("Doubled:", doubled);

const students = [
  { name: "Alex", score: 85 },
  { name: "Michael", score: 72 },
  { name: "Lumen", score: 91 },
];

const names = students.map(s => s.name);
const grades = students.map(s => ({
  name: s.name,
  grade: s.score >= 80 ? "A" : "B"
}));

console.log("Student names:", names);
console.log("Grades:", grades);


// 6. ARRAYS + FILTER
console.log("\n--- 6. Arrays + filter() ---");

const evens = numbers.filter(n => n % 2 === 0);
const odds = numbers.filter(n => n % 2 !== 0);
const bigNums = numbers.filter(n => n > 3);

console.log("Even numbers:", evens);
console.log("Odd numbers:", odds);
console.log("Numbers > 3:", bigNums);

const topStudents = students.filter(s => s.score >= 80);
console.log("Top students:", topStudents);


// 7. SPREAD OPERATOR
console.log("\n--- 7. Spread Operator ---");

const arr1 = [1, 2, 3];
const arr2 = [4, 5, 6];
const combined = [...arr1, ...arr2];
console.log("Combined arrays:", combined);

const copy = [...arr1];
copy.push(99);
console.log("Original arr1:", arr1);
console.log("Copy:", copy);

const defaults = { theme: "dark", lang: "en", fontSize: 14 };
const userPrefs = { lang: "np", fontSize: 16 };
const finalSettings = { ...defaults, ...userPrefs };
console.log("Final settings:", finalSettings);

const nums = [3, 1, 4, 1, 5, 9, 2];
console.log("Max:", Math.max(...nums));
console.log("Min:", Math.min(...nums));


// 8. REAL-WORLD EXAMPLE
console.log("\n--- Real-world Example ---");

const products = [
  { id: 1, name: "Laptop",  price: 80000, inStock: true },
  { id: 2, name: "Phone",   price: 35000, inStock: true },
  { id: 3, name: "Tablet",  price: 45000, inStock: false },
  { id: 4, name: "Monitor", price: 25000, inStock: true },
  { id: 5, name: "Headset", price: 5000,  inStock: false },
];

const affordable = products
  .filter(p => p.inStock)
  .map(p => ({ ...p, discountedPrice: p.price * 0.9 }))
  .filter(p => p.discountedPrice < 50000);

console.log("Affordable discounted items:", affordable);


// 9. forEach, reduce, find, some, every
console.log("\n--- 9. Bonus Methods ---");

numbers.forEach(n => {
  if (n % 2 === 0) console.log(`${n} is even`);
});

const sum = numbers.reduce((total, current) => total + current, 0);
console.log("Sum:", sum);

const totalScore = students.reduce((acc, s) => acc + s.score, 0);
console.log("Average score:", totalScore / students.length);

// find — first match
const found = students.find(s => s.name === "Hari");
console.log("Found student:", found);

// some — at least one passes condition
console.log("Any student with score < 70?", students.some(s => s.score < 70));

// every — all must pass condition
console.log("All students scored >= 70?", students.every(s => s.score >= 70));